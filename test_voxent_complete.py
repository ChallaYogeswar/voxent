...
try:
    ...
    print(f"  - GPU Batch Size: {config.get('batch_size_gpu', 8)}")
...
try:
    ...
    mock_files = [
        "data/input_calls/call_001.wav",  # Will simulate as existing
        "data/input_calls/call_002.wav",
        "data/input_calls/call_003.wav",
    ]
    ...
    if os.path.exists(input_dir):
        ...
        if files:
            ...
            print("ℹ️  No audio files in input_calls directory (expected for first run)")
    else:
...
NEXT STEPS:
  1. Add sample audio files to: data/input_calls/
