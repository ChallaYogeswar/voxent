...
def process_file(file_path, cfg, separator=None):
    ...
    try:
        ...
        try:
            separation_result = separator.separate_vocals(audio, cfg["sample_rate"])
        ...
        try:
            ...
            vocal_label, vocal_conf = classifier.classify(vocals, cfg["sample_rate"])
            
            # Skip if too short
            if len(vocals) / cfg["sample_rate"] >= cfg["min_segment_duration"]:
                ...
                try:
                    pitch = pitch_estimator.estimate_pitch(vocals, cfg["sample_rate"])
                except:
                    pitch = 0.0
                ...
                saved_path = save_sample(vocals, cfg["sample_rate"], vocal_label, name, "data/voice_dataset")
        ...
        if np.mean(accompaniment ** 2) > 0.001:  # Check if accompaniment has energy
            ...
            try:
                ...
                accomp_label, accomp_conf = classifier.classify(accompaniment, cfg["sample_rate"])
                
                # Skip if too short
                if len(accompaniment) / cfg["sample_rate"] >= cfg["min_segment_duration"]:
                    # Estimate pitch for metadata
                    from classification.pitch_gender import PitchGenderClassifier
                    pitch_estimator = PitchGenderClassifier()
                    try:
                        pitch = pitch_estimator.estimate_pitch(accompaniment, cfg["sample_rate"])
                    except:
                        pitch = 0.0
